...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s canadaecoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  canadaecoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded canadaecoin transaction") + "\n" +
              "  canadaecoin-tx [options] -create [commands]   " + _("Create hex-encoded canadaecoin transaction") + "\n" +
              "\n";
