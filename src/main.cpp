...
#if defined(NDEBUG)
# error "Canada eCoin cannot be compiled without assertions."
#endif
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s CDN additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("canadaecoin-scriptch");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // for (int32_t version = 4; version <= 6; ++version) // Canada eCoin check for version 3, 4 and 5 upgrades
    //     if ((block.nVersion & 0xFF) < VERSIONBITS_TOP_BITS
...
static bool IsSuperMajority(int minVersion, const CBlockIndex* pstart, unsigned nRequired, const Consensus::Params& consensusParams)
{
    ...
    {
        // Canada eCoin mask off the Chain_ID and AuxPow version
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
	// Canada eCoin: Didn't 25x value due to integer wrap around
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams)
{
    ...
    LogPrint("net", "Command \"%s\" from peer=%d\n", SanitizeString(strCommand), pfrom->id);
