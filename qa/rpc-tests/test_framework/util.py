...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "canadaecoin.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    ...
    while True:
        if process.poll() is not None:
            raise Exception('canadaecoind exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("VIACOIND", "canadaecoind"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
            bitcoind_processes[i] = subprocess.Popen(args)
            if os.getenv("PYTHON_DEBUG", ""):
                print("initialize_chain: canadaecoind started, waiting for RPC to come up")
