Canada eCoin Core version 0.13.2 is now available from:

  <https://github.com/canada-ecoin/ecoincore/releases>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/canada-ecoin/ecoincore/issues>
...
updates will be released anymore. Without security updates, using a canadaecoin
...
ECDSA signatures inside Canada eCoin transactions now use validation using
...
Previous versions of Canada eCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Canada eCoin Core was to raise the default minimum
relay fee.

Canada eCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Canada eCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running canadaecoind without having to do any manual configuration.
...
Canada eCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Canada eCoin Core will relay transactions with
...
entirely in Canada eCoin Core 0.13, and it is at this time undecided whether the
...
Canada eCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Canada eCoin Core automatically creates a hidden service to listen on, without
manual configuration. Canada eCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Canada eCoin Core is listening, and
...
Canada eCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Canada eCoin
Core to use floating fees. By default, Canada eCoin Core will use floating
fees.
...
At all times, Canada eCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) VIA.
Furthermore, Canada eCoin Core will never create transactions paying less than
...
`canadaecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /canadaecoinrpc>
        ...
        # optional bypass canadaecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< canadaecoinrpc:<password>
    </Location>
...
Restarting canadaecoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in canadaecoin.conf
...
canadaecoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/canadaecoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Canada eCoin Core code base has
...
- `canadaecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `canadaecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Canada eCoin scripting system that in combination with
...
Canada eCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Canada eCoin Core.
...
The two phases are distinct in the CanadaeCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Canada eCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Canada eCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Canada eCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of canadaecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of canadaecoins being spent is exactly
...
- **Increased security for multisig:** Canada eCoin addresses (both P2PKH addresses
  ...
  believed to be provided by Canada eCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Canada eCoin's current ledger (called the UTXO set),
  ...
  note that Canada eCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Canada eCoin users to individually opt-in to almost any change in the Canada eCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Canada eCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Canada eCoin
client to Canada eCoin Core 0.13.2 to signal for segwit support. When a
...
Since Canada eCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `canadaecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `canadaecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
